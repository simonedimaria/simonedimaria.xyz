...
The challenge setup is pretty simple, there's a `proxy/app.py` server that acts as instancer and reverse proxy for the NextJS app that can be launched via the `/start` endpoint. The NextJS app is pretty much just a template that hardcodes the flag in the html at `/admin/flag`, but with the following middleware:
...
The challenge was running an hardcoded version of NextJS 15.2.3, meaning that the CVE-2025-29927 was not directly exploitable anymore.  
...
The commit message alone doesn't inspire much confidence, it looks almost it's a routine dev fix and not a critical security patch. At that time i decided to take a closer look and so i found myself reading the NextJS source code at 3AM of a random Sunday.  
In fact, I quickly realized that the "patch" wasn't actually a fix, but rather a workaround: `x-middleware-subrequest` was still allowed from ingress requests, but now a 8-byte cryptographically-random `middlewareSubrequestId` is generated and the header gets dropped if a `x-middleware-subrequest-id` doesn't match that id.  
...
![sus nextjs CVE-2025-29927 patch](./img/ninjed_nextjs_cve.png "sus nextjs CVE-2025-29927 patch")
...
So, yeah, I was right and I basically missed on an easy CVE + bounty 😭  
...
The endpoints look intended to set a CSRF token as env var to pass it later to the NextJS app, but the implementation is pretty bad. What actually happens here it that we can set any env var with a minimum length of 20 chars and a maximum of 30. Furthermore, we don't control its value, since the value is the random hex token `CSRF_TOKEN` generated by `token_hex(16)`, at the beginning of `app.py`.  
What we can do however is set any env var of that length constraint that will be interpreted as on-off switch.  

Being the NextJS app started using `subprocess.run`, the environ of the `proxt/app.py` process is the same used at the time of `npm run start` command and thus `next start` command, meaning that every env var set via the `/csrf` endpoint will be propagated to the runtime env vars of the NextJS app.  
...
With `NEXT_PRIVATE_TEST_HEADERS` set, `x-middleware-subrequest` isn't deleted anymore, even without the `x-middleware-subrequest-id` header, meaning it's CVE-2025-29927 all over again!  
